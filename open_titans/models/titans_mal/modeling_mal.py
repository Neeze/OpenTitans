...
class SlidingWindowAttention(nn.Module):
    ...
    def forward(self, seq, attention_mask=None, disable_flex_attn=False):
        ...
        if (
            ...
            and not disable_flex_attn
            and flex_attention is not None
        ):
            persist = self.num_persist_mem_tokens
            window = self.window_size

            def _mal_mask(_, __, q_idx, kv_idx):
                is_persist = kv_idx < persist
                causal = q_idx >= kv_idx
                sliding = (q_idx - kv_idx) <= window
                return causal & (is_persist | sliding)

            block_mask = create_block_mask(
                _mal_mask, B=None, H=None,
        ...
        for layer in layers:
            ...
            if layer in neural_memory_layers:
                ...
                mem = NeuralMemory(
                    ...
                    accept_weight_residual=(
                        config.neural_mem_weight_residual and not is_first_neural_mem
                    ),
                    **neural_memory_kwargs,
                )
            ...
            ff = FeedForward(
                dim=config.hidden_size,
                mult=config.intermediate_size // config.hidden_size,
            )
